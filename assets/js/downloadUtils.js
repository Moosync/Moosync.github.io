...
function getLatestAsset (os, assets) {
  return assets.filter((val) => !val.name.match('blockmap|yml') && val.name.match(os))
}
...
export async function setupDownloadButton () {
  ...
  if (os && os !== OSEnum.UNDEFINED) {
    ...
    for (const release of releases) {
      ...
      button.innerHTML = button.innerHTML.replace('${version}', `${release.version} ${releases.length > 0 ? release.ext : ''}`)
    ...
    downloadParent.innerHTML = "Sorry Moosync is not available for your platform yet"
  }
}